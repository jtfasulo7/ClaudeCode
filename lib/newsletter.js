...
const RESEARCH_PROMPT = `You are an AI newsletter writer for the Fasulostudio weekly AI newsletter. Your job is to do DEEP web research to find the most important, specific, and surprising AI developments. Return them in a structured JSON format.

TODAY'S DATE: ${new Date().toISOString().split('T')[0]}

HARD CONSTRAINT — 7-DAY WINDOW:
This newsletter ONLY covers events from the last 7 days. Calculate the exact date range: from ${new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]} to ${new Date().toISOString().split('T')[0]}. If something happened outside this window, do NOT include it — no matter how interesting. A tool released 10 days ago is stale. An announcement from 2 weeks ago is old news. Be ruthless about this cutoff.
...
<div style="background-color:#050505;padding:0;margin:0;font-family:Georgia,'Times New Roman',Times,serif;">
  <div style="max-width:620px;margin:0 auto;padding:60px 20px;">
    ...
    <div style="text-align:center;padding-bottom:40px;border-bottom:1px solid #06b6d4;">
      <h1 style="margin:0 0 10px;font-size:32px;font-weight:400;letter-spacing:0.08em;text-transform:uppercase;color:#ffffff;font-family:Georgia,'Times New Roman',Times,serif;">Fasulostudio</h1>
    ...
    <div style="border-top:1px solid #1a1a1a;padding:32px 8px 0;text-align:center;">
      <p style="margin:0 0 4px;font-size:10px;letter-spacing:0.25em;text-transform:uppercase;color:#3a3a3a;">Fasulostudio &middot; jtfasulo.com</p>
